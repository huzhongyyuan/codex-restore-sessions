...
Use `scripts/provision_codex.py` only when the user wants profiles, shell wrappers, a shared session
...
```bash
cp assets/spec.example.toml <host>.toml
...
template does not pin a model version; omit `model` to inherit the installed Codex default.
